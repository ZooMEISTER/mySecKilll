...
public class SeckillServiceImpl implements SeckillService {
    ...
    public List<UserBean> findAllUsers(){
        List<UserBean> userList = redisTemplate.boundHashOps(key_userlist).values();
        System.out.println("redissize" + redisTemplate.boundHashOps(key_userlist).size());
        if (userList == null || userList.size() == 0){
            userList = seckillMapper.findAllUsers();
            for (UserBean user : userList){
                //将秒杀列表数据依次放入redis缓存中，key:秒杀表的ID值；value:秒杀商品数据
                redisTemplate.boundHashOps(key_userlist).put(user.getUsername(), user);
                logger.info("findAllUsers -> 从数据库中读取放入缓存中");
            }
        }
        else{
            logger.info("findAllUsers -> 从缓存中读取用户数据");
        }
